#!/usr/bin/env bash
# Phase 5 GPU continuation — lever attempts after re-baseline + profile.
set -euo pipefail
...
LOG="$OPT_OUT/phase5-gpu-continue.log"
...
exec > >(tee -a "$LOG") 2>&1
echo "=== Phase 5 GPU continuation $(date -Is) baseline=$BASELINE_ID commit=$GIT_COMMIT ==="
...
for name in ("lever-attempt-table.json", "phase5-fast-path.json", "phase5-backlog.json", "bottleneck-report.json", "compare-summary.json"):
    p = out / name
    if p.is_file():
...
echo "=== Phase 5 GPU continuation finished $(date -Is) ==="
