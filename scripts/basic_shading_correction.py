...
"""Shading correction algorithm based on the BaSiC method"""
...
def _build_arg_parser():
    ...
    p.add_argument("directory", help="Full path a directory containing the tiles to process")
    p.add_argument("output_directory", help="Full path to a directory where the output will be saved")
    p.add_argument("--extension", default=".tif", help="Image tile extension (default=%(default)s)")
    p.add_argument("--estimate_darkfield", action="store_true",
                        help="Estimate the darkfield in addition to the flatfield.")
    p.add_argument("--apply_correction", action="store_true",
                        help="Apply the shading correction with the estimated or loaded profiles.")
    p.add_argument("--use_flatfield", default=None, help="Use existing flatfield (skip estimation)")
    p.add_argument("--use_darkfield", default=None, help="Use existing darkfield (skip estimation)")
    p.add_argument("--epsilon", default=1e-6, type=float,
                        help="Stability coefficient to use for the shading correction. (default=%(default)s)")
    p.add_argument("--l_s", default=None, type=float,
                        help="Flat-field regularization parameter (set automatically if None)")
    p.add_argument("--l_d", default=None, type=float,
                        help="Dark-field regularization parameter (set automatically if None)")
    p.add_argument("--output_flatfield_filename", default=None,
                        help="Optional output flatfield filename (if none, flatfield.tif will be saved in the output directory).")
    p.add_argument("--output_darkfield_filename", default=None,
                        help="Optional output flatfield filename (if none, darkfield.tif will be saved in the output directory).")
